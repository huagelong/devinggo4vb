...
> **注意**: 本项目为纯后端服务，前端项目（Vue 3 + Arco Design）需要独立部署，详见[子模块地址](#子模块地址)。
...
- 🌍 **国际化**: 内置多语言支持，轻松实现国际化
- 🔌 **前后端分离**: RESTful API设计，支持任何前端框架对接
...
本仓库为后端API服务，前端项目需要独立部署：

- **后端项目（本仓库）**: https://github.com/huagelong/devinggo
- **后台管理前端**: [devingadmin](https://github.com/huagelong/devingadmin) - Vue 3 + Arco Design
- **官网前端**: [devingsite](https://github.com/huagelong/devingsite) - Nuxt 3
...
### 前端技术栈（独立项目）
前端项目独立维护，支持与本后端API对接：
- **核心框架**: Vue 3.4 - 渐进式JavaScript框架
- **构建工具**: Vite 5+ - 下一代前端构建工具
- **UI组件库**: Arco Design - 字节跳动企业级设计系统
- **状态管理**: Pinia - Vue官方状态管理库
- **前端仓库**: [devingadmin](https://github.com/huagelong/devingadmin)
...
### 前端环境（如需部署前端）
- **Node.js**: >= 18.0
- 详见 [devingadmin 项目](https://github.com/huagelong/devingadmin)
...
> **注意**: 如果需要部署前端项目，还需要安装 Node.js >= 18.0
...
> **前端部署**: 如需部署前端项目，请参考 [devingadmin 仓库](https://github.com/huagelong/devingadmin) 的部署说明。
...
**前端访问**: 
- 需要独立部署前端项目，详见 [devingadmin](https://github.com/huagelong/devingadmin)
- 前端配置API地址为: http://localhost:8070
- 前端默认账号: superAdmin / admin123
...
```bash
# 代码生成相关
go run hack/generator/main.go crud:create --module=system --business=user --table=system_user   # 生成CRUD代码
...
go run hack/generator/main.go module:export --name=system --output=./backup                     # 导出模块
go run hack/generator/main.go module:import --file=./backup/system.zip                          # 导入模块
...
go run hack/generator/main.go crud:create \
  --module=system \
  --business=user \
  --table=system_user \
  --name="用户" \
...
> **注意**: 前端页面需要在 [devingadmin](https://github.com/huagelong/devingadmin) 项目中单独开发
...
本项目为纯后端API服务，前端需要单独部署并配置API地址。

#### 1. 部署后端

按照[快速开始](#快速开始)部署后端服务，默认运行在 `http://localhost:8070`

#### 2. 部署前端

```bash
# 克隆前端项目
git clone https://github.com/huagelong/devingadmin.git
cd devingadmin
...
**Q: 如何部署前端项目？**  
A: 前端项目独立于本仓库，请参考 [devingadmin](https://github.com/huagelong/devingadmin) 的部署文档。需要在前端项目中配置后端API地址。
...
**前端技术（独立项目）**
5. [MineAdmin-Vue V2](https://docv2.mineadmin.com/) - 前端基础框架
6. [Vue3](https://v3.cn.vuejs.org/) - 渐进式JavaScript框架
7. [Arco Design](https://arco.design/) - 企业级UI组件库
8. [Vite](https://cn.vitejs.dev/) - 下一代前端构建工具
9. [Pinia](https://pinia.vuejs.org/) - Vue状态管理库
